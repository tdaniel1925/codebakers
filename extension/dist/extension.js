...
${t instanceof Error?t.message:"Tool execution failed"}`,timestamp:new Date})}finally{this._panel?.webview.postMessage({type:"typing",isTyping:!1}),this._updateWebview()}}async _applyFileOperation(e){if(!this._panel)return;let t=this._pendingChanges.find(n=>n.id===e);if(!(!t||t.status!=="pending"))try{if(await this.fileOps.applyChange({path:t.operation.path,action:t.operation.action,content:t.operation.content,description:t.operation.description}))t.status="applied",w.window.showInformationMessage(`\u2705 ${t.operation.action}: ${t.operation.path}`),this._updatePendingChanges(),t.operation.action!=="delete"&&await this.fileOps.openFile(t.operation.path),setTimeout(()=>{this._pendingChanges=this._pendingChanges.filter(s=>s.id!==e),this._updatePendingChanges()},1500);else throw new Error("Operation failed")}catch(n){w.window.showErrorMessage(`\u274C Failed: ${t.operation.path} - ${n}`),this._updatePendingChanges()}}async _applyAllPendingChanges(){if(!this._panel)return;let e=this._pendingChanges.filter(i=>i.status==="pending");if(e.length===0){w.window.showInformationMessage("No pending changes to apply");return}let t=e.length;this._panel?.webview.postMessage({type:"showProgress",text:`Applying 0/${t} files...`,current:0,total:t,show:!0});let n=5,s=0,o=0;for(let i=0;i<e.length;i+=n){let r=e.slice(i,i+n);(await Promise.allSettled(r.map(async l=>{try{return await this.fileOps.applyChange({path:l.operation.path,action:l.operation.action,content:l.operation.content,description:l.operation.description})?(l.status="applied",!0):!1}catch(c){return console.error(`Failed to apply ${l.operation.path}:`,c),!1}}))).forEach(l=>{l.status==="fulfilled"&&l.value?s++:o++});let p=s+o;this._panel?.webview.postMessage({type:"showProgress",text:`Applying ${p}/${t} files...`,current:p,total:t,show:!0}),this._updatePendingChanges()}this._panel?.webview.postMessage({type:"showProgress",show:!1}),w.window.showInformationMessage(`\u2705 Applied ${s} file(s)${o>0?`, ${o} failed`:""}`),this._pendingChanges=this._pendingChanges.filter(i=>i.status==="rejected"),this._updatePendingChanges(),s>0&&await this._runTscCheck()}async _runTscCheck(e=!0){if(await this.fileOps.fileExists("tsconfig.json")){this._panel?.webview.postMessage({type:"showStatus",text:"Checking TypeScript...",show:!0});try{let s=await new ba().runTypeScriptCheck();if(this._panel?.webview.postMessage({type:"showStatus",show:!1}),!s.passed&&s.errors.length>0){this._panel?.webview.postMessage({type:"rippleCheck",errors:s.errorCount,warnings:0,details:s.errors.slice(0,4).map(i=>({type:"error",message:`${i.file}:${i.line} - ${i.message.substring(0,60)}${i.message.length>60?"...":""}`}))});let o=await w.window.showWarningMessage(`\u26A0\uFE0F TypeScript errors found (${s.errorCount})`,"Auto-Fix with AI","Show Errors","Ignore");if(o==="Auto-Fix with AI"&&e){let i=`Please fix these TypeScript errors:

${s.errors.map(r=>`${r.file}:${r.line}: ${r.message}`).join(`
`)}`;this._panel?.webview.postMessage({type:"showStatus",text:"AI fixing TypeScript errors...",show:!0}),await this.sendMessage(i)}else if(o==="Show Errors"){let i=w.window.createOutputChannel("CodeBakers TSC");i.clear(),i.appendLine("TypeScript Errors:"),i.appendLine(`=================
`),s.errors.forEach(r=>{i.appendLine(`${r.file}:${r.line}:${r.column}`),i.appendLine(`  ${r.message}
`)}),i.show()}}else this._panel?.webview.postMessage({type:"rippleCheck",errors:0,warnings:0,details:[{type:"pass",message:"TypeScript compilation successful"}]}),w.window.showInformationMessage("\u2705 TypeScript check passed!")}catch(n){this._panel?.webview.postMessage({type:"showStatus",show:!1}),console.error("TSC check failed:",n)}}}_rejectFileOperation(e){let t=this._pendingChanges.find(n=>n.id===e);t&&t.status==="pending"&&(t.status="rejected",this._updatePendingChanges())}_rejectAllPendingChanges(){for(let e of this._pendingChanges)e.status==="pending"&&(e.status="rejected");this._updatePendingChanges()}async _runCommand(e){if(!this._panel)return;let t=this._pendingCommands.find(n=>n.id===e);if(!(!t||t.status!=="pending")){try{t.status="running",this._updatePendingChanges(),await this.fileOps.runCommand(t.command.command,t.command.description||"CodeBakers"),t.status="done",w.window.showInformationMessage(`\u{1F680} Running: ${t.command.command}`)}catch(n){t.status="pending",w.window.showErrorMessage(`\u274C Failed to run command: ${n}`)}this._updatePendingChanges()}}async _showDiff(e){let t=this._pendingChanges.find(n=>n.id===e);if(!(!t||!t.operation.content))try{await this.fileOps.showDiff(t.operation.path,t.operation.content,`CodeBakers: ${t.operation.path}`)}catch(n){w.window.showErrorMessage(`\u274C Failed to show diff: ${n}`)}}async _undoFileOperation(e){let t=this._pendingChanges.find(n=>n.id===e);if(!t||t.status!=="applied"){w.window.showWarningMessage("Cannot undo - change was not applied");return}try{await this.fileOps.restoreFromBackup(t.operation.path)&&(t.status="pending",this._updatePendingChanges())}catch(n){w.window.showErrorMessage(`\u274C Failed to undo: ${n}`)}}async _undoAppliedFiles(e){if(!e||e.length===0)return;let t=0;for(let n of e)try{await this.fileOps.restoreFromBackup(n)&&(t++,this._pendingChanges=this._pendingChanges.filter(o=>o.operation.path!==n))}catch(s){console.error(`Failed to undo ${n}:`,s)}t>0&&(w.window.showInformationMessage(`\u21A9\uFE0F Undone ${t} file${t>1?"s":""}`),this._updatePendingChanges())}_cancelCurrentRequest(){this._abortController&&(this._abortController.abort(),this._abortController=null,this._panel?.webview.postMessage({type:"requestCancelled"}),w.window.showInformationMessage("Request cancelled"))}_logProjectTime(e){let t=w.workspace.workspaceFolders?.[0];if(!t)return;let n=require("fs"),s=require("path"),o=t.uri.fsPath,i=s.join(o,".codebakers"),r=s.join(i,"timelog.json");n.existsSync(i)||n.mkdirSync(i,{recursive:!0});let d={sessions:[],totals:{totalMinutes:0,totalCost:0,totalRequests:0,totalTokens:0}};if(n.existsSync(r))try{d=JSON.parse(n.readFileSync(r,"utf-8"))}catch{}let p=new Date().toISOString().split("T")[0],l=Math.round(e.activeTime/6e4),c=d.sessions.find(u=>u.date===p);c||(c={date:p,activeMinutes:0,cost:0,requests:0,tokens:0},d.sessions.push(c)),c.activeMinutes=Math.max(c.activeMinutes,l),c.cost=Math.max(c.cost,e.totalCost),c.requests=Math.max(c.requests,e.requests),c.tokens=Math.max(c.tokens,e.tokens),d.totals={totalMinutes:d.sessions.reduce((u,g)=>u+g.activeMinutes,0),totalCost:d.sessions.reduce((u,g)=>u+g.cost,0),totalRequests:d.sessions.reduce((u,g)=>u+g.requests,0),totalTokens:d.sessions.reduce((u,g)=>u+g.tokens,0)};let m=new Date;m.setDate(m.getDate()-90),d.sessions=d.sessions.filter(u=>new Date(u.date)>=m),n.writeFileSync(r,JSON.stringify(d,null,2))}async _deployToVercel(){let e=w.workspace.workspaceFolders?.[0];if(!e){w.window.showErrorMessage("No workspace folder open");return}this._panel?.webview.postMessage({type:"showStatus",show:!0,text:"\u{1F680} Deploying to Vercel..."}),this._messages.push({role:"assistant",content:"\u{1F680} **Starting Vercel deployment...**\n\nRunning `vercel --prod`...",timestamp:new Date}),this._updateWebview();try{let t=require("child_process"),n=e.uri.fsPath,s=await new Promise((d,p)=>{t.exec("npx vercel --prod --yes",{cwd:n,timeout:3e5},(l,c,m)=>{l&&!c?p(l):d({stdout:c,stderr:m})})}),o=s.stdout.match(/https:\/\/[^\s]+\.vercel\.app/),i=o?o[0]:null,r=`\u2705 **Deployment successful!**

`;i&&(r+=`\u{1F517} **Live URL:** [${i}](${i})

`),r+="```\n"+s.stdout.slice(-500)+"\n```",this._messages.push({role:"assistant",content:r,timestamp:new Date}),w.window.showInformationMessage(`\u2705 Deployed successfully!${i?` URL: ${i}`:""}`,"Open URL").then(d=>{d==="Open URL"&&i&&w.env.openExternal(w.Uri.parse(i))})}catch(t){let n=t.message||"Unknown error";n.includes("vercel")&&n.includes("not found")?this._messages.push({role:"assistant",content:"\u274C **Vercel CLI not found**\n\nPlease install it first:\n```bash\nnpm install -g vercel\n```\n\nThen run `vercel login` to authenticate.",timestamp:new Date}):this._messages.push({role:"assistant",content:`\u274C **Deployment failed**
...
3. Project linked (\`vercel link\`)`,timestamp:new Date}),w.window.showErrorMessage(`Deployment failed: ${n}`)}finally{this._panel?.webview.postMessage({type:"showStatus",show:!1}),this._updateWebview()}}async _pushToGitHub(){let e=w.workspace.workspaceFolders?.[0];if(!e){w.window.showErrorMessage("No workspace folder open");return}let t=e.uri.fsPath,n=require("child_process");try{await new Promise((i,r)=>{n.exec("git rev-parse --git-dir",{cwd:t},d=>{d?r(d):i()})})}catch{w.window.showErrorMessage("Not a git repository. Run `git init` first.");return}let s=!1;try{s=(await new Promise((r,d)=>{n.exec("git status --porcelain",{cwd:t},(p,l)=>{p?d(p):r(l)})})).trim().length>0}catch(i){w.window.showErrorMessage(`Git error: ${i.message}`);return}let o="";if(s){let i=await w.window.showInputBox({prompt:"Enter commit message (or leave empty to skip commit)",placeHolder:"feat: add new feature",value:"chore: update from CodeBakers"});if(i===void 0)return;o=i}this._panel?.webview.postMessage({type:"showStatus",show:!0,text:"\u{1F4E4} Pushing to GitHub..."}),this._messages.push({role:"assistant",content:s&&o?`\u{1F4E4} **Pushing to GitHub...**
...
`),p+="```\n"+i.slice(-300)+"\n```",this._messages.push({role:"assistant",content:p,timestamp:new Date}),w.window.showInformationMessage("\u2705 Pushed to GitHub!",d.includes("github.com")?"Open Repository":void 0).then(l=>{l==="Open Repository"&&d&&w.env.openExternal(w.Uri.parse(d))})}catch(i){let r=i.message||"Unknown error";r.includes("no upstream")||r.includes("no tracking")?this._messages.push({role:"assistant",content:"\u274C **No upstream branch set**\n\nRun this to set up tracking:\n```bash\ngit push -u origin main\n```",timestamp:new Date}):r.includes("Permission denied")||r.includes("authentication")?this._messages.push({role:"assistant",content:`\u274C **Authentication failed**
...
\`\`\`
${r}
\`\`\``,timestamp:new Date}),w.window.showErrorMessage(`Push failed: ${r}`)}finally{this._panel?.webview.postMessage({type:"showStatus",show:!1}),this._updateWebview()}}_openPreviewInBrowser(e=3e3){let t=`http://localhost:${e}`;w.env.openExternal(w.Uri.parse(t)),w.window.showInformationMessage(`Opening ${t} in browser...`)}async _installPackages(e){if(!e||e.length===0)return;let t=w.workspace.workspaceFolders?.[0];if(!t){w.window.showErrorMessage("No workspace folder open");return}let n=t.uri.fsPath,s=require("child_process"),o=e.join(" ");this._panel?.webview.postMessage({type:"toast",message:`\u{1F4E6} Installing: ${o}...`});try{await new Promise((i,r)=>{s.exec(`npm install ${o}`,{cwd:n,timeout:12e4},(d,p,l)=>{d?r(d):i()})}),this._panel?.webview.postMessage({type:"toast",message:`\u2705 Installed: ${o}`}),w.window.showInformationMessage(`\u2705 Installed: ${o}`)}catch(i){let r=i.message||"Unknown error";this._panel?.webview.postMessage({type:"toast",message:`\u274C Install failed: ${r}`}),w.window.showErrorMessage(`Package install failed: ${r}`)}}_loadTeamNotes(){let e=w.workspace.workspaceFolders?.[0];if(!e)return;let t=require("fs"),s=require("path").join(e.uri.fsPath,".codebakers","team-notes.md"),o="";if(t.existsSync(s))try{o=t.readFileSync(s,"utf-8")}catch{}this._panel?.webview.postMessage({type:"updateTeamNotes",notes:o})}_saveTeamNotes(e){let t=w.workspace.workspaceFolders?.[0];if(!t)return;let n=require("fs"),s=require("path"),o=s.join(t.uri.fsPath,".codebakers"),i=s.join(o,"team-notes.md");n.existsSync(o)||n.mkdirSync(o,{recursive:!0}),n.writeFileSync(i,e)}async _analyzeImpact(e){if(!this._panel)return;let t=w.workspace.workspaceFolders?.[0];if(!t){this._panel.webview.postMessage({type:"impactAnalysis",files:[{name:"No workspace open",impact:"medium"}],warning:"Open a workspace folder to enable impact analysis"});return}let n=require("fs"),s=require("path"),o=e.toLowerCase(),i=[],r="";try{let d=s.join(t.uri.fsPath,"src");if(n.existsSync(d)){if(o.includes("api")||o.includes("endpoint")||o.includes("route")){let p=s.join(d,"app","api");n.existsSync(p)&&i.push({name:"src/app/api/",impact:"high"})}if(o.includes("component")||o.includes("button")||o.includes("form")||o.includes("modal")){let p=s.join(d,"components");n.existsSync(p)&&i.push({name:"src/components/",impact:"medium"})}if(o.includes("page")||o.includes("layout")){let p=s.join(d,"app");n.existsSync(p)&&i.push({name:"src/app/",impact:"high"})}if(o.includes("database")||o.includes("schema")||o.includes("table")||o.includes("model")){let p=s.join(d,"lib","db","schema.ts");n.existsSync(p)&&(i.push({name:"src/lib/db/schema.ts",impact:"high"}),r="Database schema changes may require migrations")}if(o.includes("auth")||o.includes("login")||o.includes("session")){let p=s.join(d,"lib","auth.ts");n.existsSync(p)&&(i.push({name:"src/lib/auth.ts",impact:"high"}),r="Authentication changes require careful security review")}(o.includes("style")||o.includes("css")||o.includes("theme"))&&i.push({name:"*.css / globals.css",impact:"medium"})}i.length===0&&i.push({name:"src/",impact:"medium"})}catch(d){console.error("Impact analysis error:",d),i.push({name:"Analysis unavailable"})}this._panel.webview.postMessage({type:"impactAnalysis",files:i,warning:r})}_showAppliedToast(e){let t=e.length,n=e.map(s=>s.split("/").pop()).join(", ");this._panel?.webview.postMessage({type:"toast",message:`Applied ${t} file${t>1?"s":""}: ${n}`,action:"undo",files:e}),this._runTscCheck()}async _addPinnedFile(){let e=await w.window.showOpenDialog({canSelectMany:!0,canSelectFolders:!1,openLabel:"Add to Context",filters:{"Code Files":["ts","tsx","js","jsx","json","md","css","html","py","go","rs","java","c","cpp","h"],"All Files":["*"]}});if(!(!e||e.length===0)){for(let t of e){let n=w.workspace.asRelativePath(t);if(!this._pinnedFiles.some(s=>s.path===n))try{let s=await this.fileOps.readFile(n),o=await w.workspace.fs.stat(t);this._pinnedFiles.push({path:n,name:n.split("/").pop()||n,content:s||"",size:o.size})}catch(s){console.error(`Failed to read file ${n}:`,s),w.window.showWarningMessage(`Could not read: ${n}`)}}this._updatePinnedFiles(),w.window.showInformationMessage(`\u{1F4CE} Added ${e.length} file(s) to context`)}}_removePinnedFile(e){this._pinnedFiles=this._pinnedFiles.filter(t=>t.path!==e),this._updatePinnedFiles()}_updatePinnedFiles(){this._panel&&this._panel.webview.postMessage({type:"updatePinnedFiles",files:this._pinnedFiles.map(e=>({path:e.path,name:e.name,size:e.size}))})}_getPinnedFilesContext(){if(this._pinnedFiles.length===0)return"";let e=`
