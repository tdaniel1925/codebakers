...
var vscode, path, fs, FileOperations, DiffContentProvider;
var init_FileOperations = __esm({
  "src/FileOperations.ts"() {
    ...
    FileOperations = class {
      ...
      constructor() {
        this._backups = /* @__PURE__ */ new Map();
        this._operationLock = /* @__PURE__ */ new Set();
       ...
       */
      async acquireLock(relativePath, timeoutMs = 5e3) {
       ...
       */
      async restoreFromBackup(relativePath) {
      ...
      cleanupOldBackups() {
        ...
        for (const [path4, backup] of this._backups) {
          if (backup.timestamp < oneHourAgo) {
            this._backups.delete(path4);
          }
        }
      }
      /**
       ...
       */
      async showDiff(relativePath, newContent, title) {
      ...
      async applyChange(change) {
        ...
        try {
          ...
          switch (change.action) {
            ...
            default:
              vscode.window.showErrorMessage(`Unknown action: ${change.action}`);
              return false;
          }
        } finally {
      ...
      async runCommandWithOutput(command) {
        return new Promise((resolve, reject) => {
          ...
          exec2(command, { cwd: this.workspaceRoot, timeout: 3e4 }, (error, stdout, stderr) => {
            if (error) {
      ...
      async findFiles(pattern, exclude) {
        ...
        return files.map((f2) => vscode.workspace.asRelativePath(f2));
      }
      /**
       * Open a file in the editor
       */
      async openFile(relativePath, selection) {
        if (!this.workspaceRoot) return;
        const fullPath = path.join(this.workspaceRoot, relativePath);
        const uri = vscode.Uri.file(fullPath);
      ...
      getCurrentFile() {
        ...
        const selection = editor.selection.isEmpty ? void 0 : editor.document.getText(editor.selection);
      ...
      async getFileTree(maxDepth = 2) {
        const files = [];
        const addFiles = async (dir, depth) => {
...
// src/CodeValidator.ts
var vscode2, fs2, path2, import_child_process, import_util, execAsync, CodeValidator;
var init_CodeValidator = __esm({
  "src/CodeValidator.ts"() {
    "use strict";
    ...
    CodeValidator = class {
      constructor() {
        this.typeInventory = null;
        this.installedPackages = /* @__PURE__ */ new Set();
      ...
      async scanInstalledPackages() {
        if (!this.workspaceRoot) return;
        const packageJsonPath = path2.join(this.workspaceRoot, "package.json");
      ...
      async scanTypeInventory() {
        ...
        for (const file of files) {
          try {
            const content = fs2.readFileSync(file.fsPath, "utf-8");
      ...
      extractExports(content, filePath) {
        ...
        while ((match = funcRegex.exec(content)) !== null) {
          exports2.push({ name: match[1], file: filePath, kind: "function" });
        }
        const constRegex = /export\s+const\s+(\w+)/g;
        while ((match = constRegex.exec(content)) !== null) {
          exports2.push({ name: match[1], file: filePath, kind: "const" });
        }
        const classRegex = /export\s+class\s+(\w+)/g;
        while ((match = classRegex.exec(content)) !== null) {
          exports2.push({ name: match[1], file: filePath, kind: "class" });
        }
        if (/export\s+default/.test(content)) {
          exports2.push({ name: "default", file: filePath, kind: "default" });
        }
        if (exports2.length > 0) {
          this.typeInventory.exports.set(filePath, exports2);
        }
      }
      /**
       * Check if required packages are installed
       */
      checkDependencies(fileOperations) {
        const result = {
          missing: [],
          available: [],
          suggestions: []
      ...
      async validateFileOperations(fileOperations) {
        ...
        const hasTestFile = fileOperations.some(
          (op) => op.path.includes(".test.") || op.path.includes(".spec.") || op.path.includes("__tests__")
        );
      ...
      validateImports(op) {
        ...
        while ((match = relativeImportRegex.exec(op.content)) !== null) {
          ...
          const opDir = path2.dirname(op.path);
          const resolvedPath = path2.join(this.workspaceRoot, opDir, importPath);
       ...
       */
      suggestExistingTypes(fileOperations) {
        const suggestions = [];
      ...
      getTypeInventoryForContext() {
        ...
        for (const [file, types] of byFile) {
          if (types.length > 0) {
            lines.push(`  ${file}:`);
            for (const t2 of types.slice(0, 10)) {
              lines.push(`    - ${t2.kind} ${t2.name}`);
            }
            if (types.length > 10) {
       ...
       */
      getInstalledPackagesForContext() {
        return Array.from(this.installedPackages).slice(0, 50);
      }
      /**
      ...
      async runTypeScriptCheck() {
        ...
        const tsconfigPath = path2.join(this.workspaceRoot, "tsconfig.json");
       ...
       */
      async checkSpecificFiles(filePaths) {
        if (!this.workspaceRoot || filePaths.length === 0) {
          return { passed: true, errors: [], errorCount: 0 };
        }
        const tsFiles = filePaths.filter((f2) => f2.endsWith(".ts") || f2.endsWith(".tsx"));
        if (tsFiles.length === 0) {
          return { passed: true, errors: [], errorCount: 0 };
        }
        try {
          const filesArg = tsFiles.map((f2) => `"${f2}"`).join(" ");
          await execAsync(`npx tsc --noEmit ${filesArg}`, {
            cwd: this.workspaceRoot,
            timeout: 3e4
            // 30 second timeout for specific files
          });
          return { passed: true, errors: [], errorCount: 0 };
        } catch (error) {
          const output = error.stdout || error.stderr || "";
...
// src/ChatPanelProvider.ts
var ChatPanelProvider_exports = {};
__export(ChatPanelProvider_exports, {
  ChatPanelProvider: () => ChatPanelProvider
});
var vscode3, ChatPanelProvider;
var init_ChatPanelProvider = __esm({
  "src/ChatPanelProvider.ts"() {
    "use strict";
    vscode3 = __toESM(require("vscode"));
    init_FileOperations();
    init_CodeValidator();
    ChatPanelProvider = class _ChatPanelProvider {
      // Update UI every 50ms max
      constructor(context, client2, projectContext2) {
        this.context = context;
        this.client = client2;
        this.projectContext = projectContext2;
        this._messages = [];
        ...
        this._lastStreamUpdate = 0;
        this.STREAM_THROTTLE_MS = 50;
        this.fileOps = new FileOperations();
      }
      static getInstance(context, client2, projectContext2) {
        if (!_ChatPanelProvider._instance) {
          _ChatPanelProvider._instance = new _ChatPanelProvider(context, client2, projectContext2);
        }
        return _ChatPanelProvider._instance;
      }
      show() {
        if (this._panel) {
          this._panel.reveal(vscode3.ViewColumn.Beside);
          return;
        }
        ...
        this._panel.iconPath = vscode3.Uri.joinPath(this.context.extensionUri, "media", "icon.svg");
...
${error instanceof Error ? error.message : "Tool execution failed"}`,
      ...
      async _applyFileOperation(id) {
        if (!this._panel) return;
        const change = this._pendingChanges.find((c2) => c2.id === id);
        ...
        try {
          ...
          if (success) {
            ...
            vscode3.window.showInformationMessage(`\u2705 ${change.operation.action}: ${change.operation.path}`);
            this._updatePendingChanges();
            if (change.operation.action !== "delete") {
              await this.fileOps.openFile(change.operation.path);
            }
            setTimeout(() => {
              this._pendingChanges = this._pendingChanges.filter((c2) => c2.id !== id);
              this._updatePendingChanges();
            }, 1500);
          } else {
          ...
          vscode3.window.showErrorMessage(`\u274C Failed: ${change.operation.path} - ${error}`);
          this._updatePendingChanges();
        }
      }
      async _applyAllPendingChanges() {
        if (!this._panel) return;
        const pending = this._pendingChanges.filter((c2) => c2.status === "pending");
        if (pending.length === 0) {
          vscode3.window.showInformationMessage("No pending changes to apply");
          return;
        }
        ...
        let success = 0;
        let failed = 0;
        for (let i2 = 0; i2 < pending.length; i2 += BATCH_SIZE) {
          const batch = pending.slice(i2, i2 + BATCH_SIZE);
        ...
        this._pendingChanges = this._pendingChanges.filter((c2) => c2.status === "rejected");
        this._updatePendingChanges();
        if (success > 0) {
          await this._runTscCheck();
        }
      }
      async _runTscCheck(autoRetry = true) {
...
${result.errors.map(
              ...
              const outputChannel = vscode3.window.createOutputChannel("CodeBakers TSC");
              outputChannel.clear();
              outputChannel.appendLine("TypeScript Errors:");
              outputChannel.appendLine("=================\n");
              result.errors.forEach((e2) => {
                outputChannel.appendLine(`${e2.file}:${e2.line}:${e2.column}`);
...
`);
            ...
            vscode3.window.showInformationMessage("\u2705 TypeScript check passed!");
          }
        } catch (error) {
          this._panel?.webview.postMessage({ type: "showStatus", show: false });
      ...
      async _runCommand(id) {
        if (!this._panel) return;
        const cmd = this._pendingCommands.find((c2) => c2.id === id);
        ...
        try {
          ...
          cmd.status = "done";
          vscode3.window.showInformationMessage(`\u{1F680} Running: ${cmd.command.command}`);
        } catch (error) {
          cmd.status = "pending";
          vscode3.window.showErrorMessage(`\u274C Failed to run command: ${error}`);
        }
        this._updatePendingChanges();
      }
      async _showDiff(id) {
        ...
        try {
          ...
          vscode3.window.showErrorMessage(`\u274C Failed to show diff: ${error}`);
        }
      }
      async _undoFileOperation(id) {
        const change = this._pendingChanges.find((c2) => c2.id === id);
        if (!change || change.status !== "applied") {
          vscode3.window.showWarningMessage("Cannot undo - change was not applied");
          return;
        }
        try {
          ...
          vscode3.window.showErrorMessage(`\u274C Failed to undo: ${error}`);
        }
      }
      _cancelCurrentRequest() {
        if (this._abortController) {
          ...
          vscode3.window.showInformationMessage("Request cancelled");
        }
      }
      /**
       ...
       */
      _logProjectTime(data) {
        const workspaceFolder = vscode3.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) return;
        const fs5 = require("fs");
        const path4 = require("path");
        const rootPath = workspaceFolder.uri.fsPath;
        const codebakersDir = path4.join(rootPath, ".codebakers");
        const timelogPath = path4.join(codebakersDir, "timelog.json");
        if (!fs5.existsSync(codebakersDir)) {
          fs5.mkdirSync(codebakersDir, { recursive: true });
        }
        let timelog = {
          sessions: [],
          totals: { totalMinutes: 0, totalCost: 0, totalRequests: 0, totalTokens: 0 }
        };
        if (fs5.existsSync(timelogPath)) {
          try {
            timelog = JSON.parse(fs5.readFileSync(timelogPath, "utf-8"));
          } catch {
          }
        }
        const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
        const activeMinutes = Math.round(data.activeTime / 6e4);
        let todaySession = timelog.sessions.find((s2) => s2.date === today);
        if (!todaySession) {
        ...
        timelog.totals = {
          totalMinutes: timelog.sessions.reduce((sum, s2) => sum + s2.activeMinutes, 0),
          totalCost: timelog.sessions.reduce((sum, s2) => sum + s2.cost, 0),
          totalRequests: timelog.sessions.reduce((sum, s2) => sum + s2.requests, 0),
          totalTokens: timelog.sessions.reduce((sum, s2) => sum + s2.tokens, 0)
        };
        const cutoff = /* @__PURE__ */ new Date();
        cutoff.setDate(cutoff.getDate() - 90);
        timelog.sessions = timelog.sessions.filter((s2) => new Date(s2.date) >= cutoff);
        fs5.writeFileSync(timelogPath, JSON.stringify(timelog, null, 2));
      }
      /**
       * Deploy to Vercel with one click
       */
      async _deployToVercel() {
        const workspaceFolder = vscode3.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
        ...
        this._panel?.webview.postMessage({
          type: "showStatus",
          show: true,
          text: "\u{1F680} Deploying to Vercel..."
        });
        this._messages.push({
          role: "assistant",
          content: "\u{1F680} **Starting Vercel deployment...**\n\nRunning `vercel --prod`...",
          timestamp: /* @__PURE__ */ new Date()
        });
        this._updateWebview();
        try {
          ...
          if (deployUrl) {
            successMsg += `\u{1F517} **Live URL:** [${deployUrl}](${deployUrl})
...
`;
          ...
          vscode3.window.showInformationMessage(
            ...
            if (selection === "Open URL" && deployUrl) {
              vscode3.env.openExternal(vscode3.Uri.parse(deployUrl));
            }
          });
        } catch (error) {
...
3. Project linked (\`vercel link\`)`,
          ...
          vscode3.window.showErrorMessage(`Deployment failed: ${errorMsg}`);
        } finally {
          this._panel?.webview.postMessage({ type: "showStatus", show: false });
          this._updateWebview();
        }
      ...
      async _pushToGitHub() {
        ...
        try {
          await new Promise((resolve, reject) => {
            cp.exec("git rev-parse --git-dir", { cwd: rootPath }, (error) => {
              if (error) reject(error);
              else resolve();
            });
          });
        } catch {
          vscode3.window.showErrorMessage("Not a git repository. Run `git init` first.");
          return;
        }
        let hasChanges = false;
        try {
          const status = await new Promise((resolve, reject) => {
            cp.exec("git status --porcelain", { cwd: rootPath }, (error, stdout) => {
              if (error) reject(error);
              else resolve(stdout);
            });
          });
          ...
          vscode3.window.showErrorMessage(`Git error: ${error.message}`);
          return;
        }
        let commitMessage = "";
        if (hasChanges) {
        ...
        this._panel?.webview.postMessage({
          type: "showStatus",
          show: true,
          text: "\u{1F4E4} Pushing to GitHub..."
        });
        this._messages.push({
          role: "assistant",
          content: hasChanges && commitMessage ? `\u{1F4E4} **Pushing to GitHub...**
...
Pushing to remote...`,
        ...
        try {
          ...
          if (hasChanges && commitMessage) {
            await new Promise((resolve, reject) => {
              cp.exec("git add -A", { cwd: rootPath }, (error) => {
                if (error) reject(error);
                else resolve();
              });
            });
            const commitResult = await new Promise((resolve, reject) => {
              cp.exec(`git commit -m "${commitMessage.replace(/"/g, '\\"')}"`, { cwd: rootPath }, (error, stdout) => {
          ...
          const pushResult = await new Promise((resolve, reject) => {
            cp.exec("git push", { cwd: rootPath, timeout: 6e4 }, (error, stdout, stderr) => {
          ...
          try {
            remoteUrl = await new Promise((resolve) => {
              cp.exec("git remote get-url origin", { cwd: rootPath }, (_error, stdout) => {
                resolve(stdout.trim());
              });
            });
          ...
          if (remoteUrl && remoteUrl.includes("github.com")) {
            successMsg += `\u{1F517} **Repository:** [${remoteUrl}](${remoteUrl})

`;
          }
          if (hasChanges && commitMessage) {
            successMsg += `\u{1F4DD} **Commit:** ${commitMessage}
...
`;
          ...
          vscode3.window.showInformationMessage(
            ...
            remoteUrl.includes("github.com") ? "Open Repository" : void 0
          ).then((selection) => {
            if (selection === "Open Repository" && remoteUrl) {
              vscode3.env.openExternal(vscode3.Uri.parse(remoteUrl));
            }
          });
        } catch (error) {
          const errorMsg = error.message || "Unknown error";
...
\`\`\``,
          ...
          vscode3.window.showErrorMessage(`Push failed: ${errorMsg}`);
        } finally {
          this._panel?.webview.postMessage({ type: "showStatus", show: false });
          this._updateWebview();
        }
      ...
      _loadTeamNotes() {
        ...
        const fs5 = require("fs");
        const path4 = require("path");
        const notesPath = path4.join(workspaceFolder.uri.fsPath, ".codebakers", "team-notes.md");
        let notes = "";
        if (fs5.existsSync(notesPath)) {
          try {
            notes = fs5.readFileSync(notesPath, "utf-8");
          } catch {
          }
        }
        this._panel?.webview.postMessage({
       ...
       */
      _saveTeamNotes(notes) {
        const workspaceFolder = vscode3.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) return;
        const fs5 = require("fs");
        const path4 = require("path");
        const codebakersDir = path4.join(workspaceFolder.uri.fsPath, ".codebakers");
        const notesPath = path4.join(codebakersDir, "team-notes.md");
        if (!fs5.existsSync(codebakersDir)) {
          fs5.mkdirSync(codebakersDir, { recursive: true });
        }
        fs5.writeFileSync(notesPath, notes);
      }
      // Pinned files management (Cursor-style context files)
      async _addPinnedFile() {
        ...
        for (const file of files) {
          const relativePath = vscode3.workspace.asRelativePath(file);
          if (this._pinnedFiles.some((f2) => f2.path === relativePath)) {
            continue;
          }
          try {
            const content = await this.fileOps.readFile(relativePath);
            const stats = await vscode3.workspace.fs.stat(file);
            ...
            console.error(`Failed to read file ${relativePath}:`, error);
            vscode3.window.showWarningMessage(`Could not read: ${relativePath}`);
          }
        }
        this._updatePinnedFiles();
        vscode3.window.showInformationMessage(`\u{1F4CE} Added ${files.length} file(s) to context`);
      }
      _removePinnedFile(path4) {
        this._pinnedFiles = this._pinnedFiles.filter((f2) => f2.path !== path4);
        this._updatePinnedFiles();
      }
      _updatePinnedFiles() {
        if (!this._panel) return;
        this._panel.webview.postMessage({
          ...
          files: this._pinnedFiles.map((f2) => ({
            path: f2.path,
            name: f2.name,
            size: f2.size
          }))
...
${oldMessages.map((m2) => `${m2.role}: ${m2.content}`).join("\n")}`;
        try {
      ...
      _updateWebview() {
        ...
        this._panel.webview.postMessage({
          type: "updateMessages",
          messages: this._messages.map((m2) => ({
            role: m2.role,
            content: m2.content,
            thinking: m2.thinking,
            timestamp: m2.timestamp.toISOString()
          }))
        });
        this._updatePendingChanges();
      }
      _updatePendingChanges() {
        if (!this._panel) return;
        const pendingFileChanges = this._pendingChanges.filter((c2) => c2.status === "pending");
        const pendingCmds = this._pendingCommands.filter((c2) => c2.status === "pending");
        this._panel.webview.postMessage({
          type: "updatePendingChanges",
          changes: this._pendingChanges.map((c2) => ({
            id: c2.id,
            path: c2.operation.path,
            action: c2.operation.action,
            description: c2.operation.description,
            status: c2.status,
            hasContent: !!c2.operation.content
          })),
          commands: this._pendingCommands.map((c2) => ({
            id: c2.id,
            command: c2.command.command,
            description: c2.command.description,
            status: c2.status
          })),
...
var init_deprecateConstructorEntries = __esm({
  "node_modules/formdata-node/lib/esm/deprecateConstructorEntries.js"() {
    ...
    deprecateConstructorEntries = (0, import_util2.deprecate)(() => {
...
var import_util3, __classPrivateFieldGet3, _FormData_instances, _FormData_entries, _FormData_setEntry, FormData2;
...
var isResponseLike, isFileLike2, isBlobLike, getStringFromMaybeBuffer, isAsyncIterableIterator, isMultipartBody;
...
var __classPrivateFieldSet6, __classPrivateFieldGet7, _PromptCachingBetaMessageStream_instances, _PromptCachingBetaMessageStream_currentMessageSnapshot, _PromptCachingBetaMessageStream_connectedPromise, _PromptCachingBetaMessageStream_resolveConnectedPromise, _PromptCachingBetaMessageStream_rejectConnectedPromise, _PromptCachingBetaMessageStream_endPromise, _PromptCachingBetaMessageStream_resolveEndPromise, _PromptCachingBetaMessageStream_rejectEndPromise, _PromptCachingBetaMessageStream_listeners, _PromptCachingBetaMessageStream_ended, _PromptCachingBetaMessageStream_errored, _PromptCachingBetaMessageStream_aborted, _PromptCachingBetaMessageStream_catchingPromiseCreated, _PromptCachingBetaMessageStream_getFinalMessage, _PromptCachingBetaMessageStream_getFinalText, _PromptCachingBetaMessageStream_handleError, _PromptCachingBetaMessageStream_beginRequest, _PromptCachingBetaMessageStream_addStreamEvent, _PromptCachingBetaMessageStream_endRequest, _PromptCachingBetaMessageStream_accumulateMessage, JSON_BUF_PROPERTY, PromptCachingBetaMessageStream;
var init_PromptCachingBetaMessageStream = __esm({
  "node_modules/@anthropic-ai/sdk/lib/PromptCachingBetaMessageStream.mjs"() {
    ...
    PromptCachingBetaMessageStream = class _PromptCachingBetaMessageStream {
      constructor() {
      ...
      [(_PromptCachingBetaMessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_listeners = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_ended = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_errored = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_aborted = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_handleError = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_instances = /* @__PURE__ */ new WeakSet(), _PromptCachingBetaMessageStream_getFinalMessage = function _PromptCachingBetaMessageStream_getFinalMessage2() {
        ...
        if (this.ended)
          return;
        __classPrivateFieldSet6(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, void 0, "f");
...
var init_prompt_caching = __esm({
  "node_modules/@anthropic-ai/sdk/resources/beta/prompt-caching/prompt-caching.mjs"() {
    ...
    init_messages2();
    init_messages2();
    PromptCaching = class extends APIResource {
      constructor() {
        super(...arguments);
...
var init_messages3 = __esm({
  "node_modules/@anthropic-ai/sdk/resources/messages.mjs"() {
    init_resource();
    init_MessageStream();
    Messages3 = class extends APIResource {
      create(body, options) {
...
// src/CodeBakersClient.ts
var CodeBakersClient_exports = {};
__export(CodeBakersClient_exports, {
  CodeBakersClient: () => CodeBakersClient
});
var vscode4, CodeBakersClient;
var init_CodeBakersClient = __esm({
  "src/CodeBakersClient.ts"() {
    "use strict";
    vscode4 = __toESM(require("vscode"));
    init_sdk();
    init_CodeValidator();
    CodeBakersClient = class {
      ...
      constructor(context) {
        ...
        this.PRICE_PER_1K_INPUT = 3e-3;
        // $3 per 1M input tokens
        this.PRICE_PER_1K_OUTPUT = 0.015;
        ...
        if (this.sessionToken && this.sessionToken.includes("%")) {
          ...
          this.sessionToken = null;
          context.globalState.update("codebakers.sessionToken", void 0);
        }
      }
      /**
      ...
      async logout() {
        this.sessionToken = null;
        this.anthropic = null;
        await this.context.globalState.update("codebakers.sessionToken", void 0);
        await this.context.globalState.update("codebakers.user", void 0);
        vscode4.window.showInformationMessage("Logged out of CodeBakers");
      }
      /**
      ...
      _calculateCost(inputTokens, outputTokens) {
        const inputCost = inputTokens / 1e3 * this.PRICE_PER_1K_INPUT;
        const outputCost = outputTokens / 1e3 * this.PRICE_PER_1K_OUTPUT;
      ...
      async handleOAuthCallback(encodedToken) {
        try {
          ...
          if (!encodedToken) {
            vscode4.window.showErrorMessage("Login failed: No token received");
            return false;
          }
          ...
          console.error("Failed to handle OAuth callback:", e2);
          console.error("Token was:", encodedToken?.substring(0, 100));
          vscode4.window.showErrorMessage(`Login failed: ${e2 instanceof Error ? e2.message : "Invalid token"}`);
          return false;
        }
      }
      async checkAuth() {
      ...
      async login() {
        try {
          ...
          const uriScheme = vscode4.env.uriScheme;
          console.log("CodeBakers: uriScheme =", uriScheme);
          const rawCallbackUri = vscode4.Uri.parse(`${uriScheme}://codebakers.codebakers/callback`);
          console.log("CodeBakers: rawCallbackUri =", rawCallbackUri.toString());
          const callbackUri = await vscode4.env.asExternalUri(rawCallbackUri);
          ...
          await vscode4.env.openExternal(vscode4.Uri.parse(loginUrl));
        ...
        try {
          ...
          if (this.sessionToken) {
            try {
              ...
              console.error("_initializeAnthropic: FAILED to decode token locally:", e2);
            }
          }
          ...
          if (response.status === 402) {
            const selection = await vscode4.window.showWarningMessage(
              rawResponse.message || "Subscribe to CodeBakers Pro ($99/month) for unlimited access",
              "Subscribe Now",
              "Start Free Trial"
            );
            if (selection === "Subscribe Now") {
              vscode4.env.openExternal(vscode4.Uri.parse(rawResponse.upgradeUrl || "https://www.codebakers.ai/dashboard/billing"));
            } else if (selection === "Start Free Trial") {
              vscode4.env.openExternal(vscode4.Uri.parse(rawResponse.trialUrl || "https://www.codebakers.ai/dashboard/billing"));
            }
            throw new Error("SUBSCRIPTION_REQUIRED");
          }
          if (!response.ok) {
            console.error("API error response:", JSON.stringify(rawResponse));
            const tokenInfo = this.sessionToken ? `token len=${this.sessionToken.length}, starts=${this.sessionToken.substring(0, 10)}` : "NO TOKEN";
          ...
          if (trial && trial.daysRemaining <= 3) {
            vscode4.window.showWarningMessage(
              ...
              if (selection === "Subscribe Now") {
                vscode4.env.openExternal(vscode4.Uri.parse("https://www.codebakers.ai/dashboard/billing"));
              }
            });
          }
      ...
      async _loadPatterns() {
        try {
          ...
          if (response.ok) {
            ...
            if (data.patterns) {
              data.patterns.forEach((p2) => this.patterns.set(p2.name, p2));
            }
          }
        } catch (error) {
      ...
      async chat(messages, projectState, callbacks, options) {
        ...
        const patternsContent = relevantPatterns.map((p2) => `## Pattern: ${p2.name}
${p2.content}`).join("\n\n");
        const systemMessages = messages.filter((m2) => m2.role === "system");
        const chatMessages = messages.filter((m2) => m2.role !== "system");
        const contextFromMessages = systemMessages.length > 0 ? "\n\n# CONTEXT\n" + systemMessages.map((m2) => m2.content).join("\n\n") : "";
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            ...
            stream.on("text", (text) => {
              ...
              fullText += text;
              const { thinking: thinking2, content: content2 } = this._parseThinkingAndContent(fullText);
              if (thinking2) {
                callbacks?.onThinking?.(thinking2);
              }
              callbacks?.onContent?.(content2);
            });
            ...
            const commands3 = this.parseCommands(content);
            ...
            const cmdCount = commands3.length;
            const patternNames = relevantPatterns.map((p2) => p2.name).join(", ") || "core";
            ...
            if (hasMissingDeps && dependencyCheck) {
              contentWithFooter += "\n\n---\n\u{1F4E6} **Missing Packages:**\n```bash\nnpm install " + dependencyCheck.missing.join(" ") + "\n```";
            }
            if (hasTscErrors && tscResult) {
              contentWithFooter += "\n\n---\n\u{1F534} **TypeScript Errors (" + tscResult.errorCount + "):**\n" + tscResult.errors.slice(0, 5).map((e2) => `- ${e2.file}:${e2.line} - ${e2.message}`).join("\n");
  ...
  ...and ${tscResult.errorCount - 5} more`;
            ...
            if (validation) {
              if (validation.errors.length > 0) {
                contentWithFooter += "\n\n---\n\u274C **Validation Errors:**\n" + validation.errors.map((e2) => `- ${e2.file}: ${e2.message}`).join("\n");
              }
              if (validation.warnings.length > 0) {
                contentWithFooter += "\n\n---\n\u26A0\uFE0F **Warnings:**\n" + validation.warnings.map((w2) => `- ${w2.file}: ${w2.message}`).join("\n");
              }
              if (validation.suggestions.length > 0) {
                contentWithFooter += "\n\n---\n\u{1F4A1} **Suggestions:**\n" + validation.suggestions.map((s2) => `- ${s2}`).join("\n");
              }
            }
            ...
            contentWithFooter += "\n\n---\n\u{1F36A} **CodeBakers** " + statusIcon + " | Files: " + fileCount + " | Commands: " + cmdCount + " | TSC: " + tscStatus + tokenDisplay + costDisplay + " | " + patternNames;
            return {
              content: contentWithFooter,
              thinking: thinking || void 0,
              fileOperations: fileOperations.length > 0 ? fileOperations : void 0,
              commands: commands3.length > 0 ? commands3 : void 0,
              projectUpdates,
              validation,
              dependencyCheck,
              usage
            ...
            if (attempt < maxRetries) {
              ...
              callbacks?.onError?.(new Error(`Request failed, retrying in ${delay / 1e3}s...`));
      ...
      parseFileOperations(text) {
        ...
        while ((match = regex.exec(text)) !== null) {
          ...
          operations.push({
            ...
            content: contentMatch ? contentMatch[1].trim() : void 0
          });
        }
        return operations;
      }
      ...
      cleanContentForDisplay(text) {
        return text.replace(/<file_operation>[\s\S]*?<\/file_operation>/g, "").replace(/<run_command>[\s\S]*?<\/run_command>/g, "").trim();
      }
      /**
       ...
       */
      _checkCompliance(fullText, thinking, fileOperations, cleanContent) {
        const issues = [];
      ...
      async executeTool(toolName, args = {}) {
        ...
        const projectPath = vscode4.workspace.workspaceFolders?.[0]?.uri.fsPath;
...
`;
      ...
      async _detectRelevantPatterns(messages) {
        ...
        for (const [pattern, keywords] of Object.entries(keywordMap)) {
          ...
          if (keywords.some((kw) => lastMessage.includes(kw))) {
            const p2 = this.patterns.get(pattern);
            if (p2) relevant.push(p2);
          }
        }
        return relevant;
      }
      _extractProjectUpdates(content) {
        ...
        if (patternMatches) {
          updates.patterns = patternMatches.flatMap(
            (m2) => m2.split(":")[1]?.split(",").map((s2) => s2.trim()) || []
          );
        }
        return Object.keys(updates).length > 0 ? updates : void 0;
      }
      _getApiEndpoint() {
        return vscode4.workspace.getConfiguration("codebakers").get("apiEndpoint", "https://www.codebakers.ai");
      }
    };
  }
});

// src/ProjectContext.ts
var ProjectContext_exports = {};
__export(ProjectContext_exports, {
  ProjectContext: () => ProjectContext
});
var vscode5, fs4, path3, ProjectContext;
  ...
  "src/ProjectContext.ts"() {
    ...
    ProjectContext = class {
      ...
      async getProjectState() {
        ...
        const rootPath = workspaceFolder.uri.fsPath;
        const state = {};
        const codebakersPath = path3.join(rootPath, ".codebakers.json");
        if (fs4.existsSync(codebakersPath)) {
          try {
            const content = fs4.readFileSync(codebakersPath, "utf-8");
        ...
        const packagePath = path3.join(rootPath, "package.json");
        if (fs4.existsSync(packagePath)) {
          try {
            const content = fs4.readFileSync(packagePath, "utf-8");
            const pkg = JSON.parse(content);
            state.packageDeps = [
        ...
        const devlogPath = path3.join(rootPath, ".codebakers", "DEVLOG.md");
        if (fs4.existsSync(devlogPath)) {
          try {
            const content = fs4.readFileSync(devlogPath, "utf-8");
        ...
        const blockedPath = path3.join(rootPath, ".codebakers", "BLOCKED.md");
        if (fs4.existsSync(blockedPath)) {
          try {
            const content = fs4.readFileSync(blockedPath, "utf-8");
      ...
      async applyUpdates(updates) {
        ...
        if (fs4.existsSync(codebakersPath)) {
          try {
            existing = JSON.parse(fs4.readFileSync(codebakersPath, "utf-8"));
          } catch {
            existing = {};
          }
        }
        ...
        if (updates.tasks) {
          ...
          existing.currentWork.lastUpdated = (/* @__PURE__ */ new Date()).toISOString();
        }
        if (updates.patterns) {
          existing.analytics = existing.analytics || {};
          existing.analytics.modulesUsed = existing.analytics.modulesUsed || {};
          updates.patterns.forEach((p2) => {
            existing.analytics.modulesUsed[p2] = (existing.analytics.modulesUsed[p2] || 0) + 1;
          });
        }
        fs4.writeFileSync(codebakersPath, JSON.stringify(existing, null, 2));
        this._cache = null;
      }
      /**
      ...
      _getDynamicContext() {
        ...
        if (!editor) return {};
        const context = {
          openFile: vscode5.workspace.asRelativePath(editor.document.uri)
        };
      ...
      _detectStack(deps) {
        ...
        if (deps.some((d2) => d2.includes("@radix-ui"))) stack.ui = "shadcn";
      ...
      async _hasTestFiles(rootPath) {
        ...
        for (const pattern of testPatterns) {
          const files = await vscode5.workspace.findFiles(pattern, "**/node_modules/**", 1);
      ...
      async _getRecentFiles(rootPath) {
        ...
        const withStats = await Promise.all(
          files.map(async (f2) => {
            try {
              const stat = await vscode5.workspace.fs.stat(f2);
              return { path: vscode5.workspace.asRelativePath(f2), mtime: stat.mtime };
            } catch {
              return null;
            }
          })
        );
        return withStats.filter((f2) => f2 !== null).sort((a2, b2) => b2.mtime - a2.mtime).slice(0, 10).map((f2) => f2.path);
      }
      /**
       * Get file tree structure for AI context
       * This helps the AI understand where files exist and where to create new ones
       */
      async _getFileTree(rootPath, maxDepth = 4) {
        ...
        const IMPORTANT_DIRS = /* @__PURE__ */ new Set([
          "src",
          "app",
          "pages",
          "components",
        ...
        const lines = [];
        const scan = (dir, prefix, depth) => {
          if (depth > maxDepth) return;
          try {
            const entries = fs4.readdirSync(dir, { withFileTypes: true });
            const dirs = entries.filter((e2) => e2.isDirectory() && !IGNORE_DIRS.has(e2.name) && !e2.name.startsWith("."));
            const files = entries.filter((e2) => e2.isFile());
            dirs.sort((a2, b2) => {
              const aImportant = IMPORTANT_DIRS.has(a2.name);
              const bImportant = IMPORTANT_DIRS.has(b2.name);
            ...
            for (let i2 = 0; i2 < dirs.length; i2++) {
              const entry = dirs[i2];
              const isLast = i2 === dirs.length - 1 && files.length === 0;
              const connector = isLast ? "\u2514\u2500\u2500 " : "\u251C\u2500\u2500 ";
              const childPrefix = isLast ? "    " : "\u2502   ";
              lines.push(`${prefix}${connector}${entry.name}/`);
              scan(path3.join(dir, entry.name), prefix + childPrefix, depth + 1);
            }
            const keyFiles = files.filter(
              (f2) => depth <= 1 ? ["package.json", "tsconfig.json", ".env.example", "next.config.js", "next.config.mjs", "drizzle.config.ts"].includes(f2.name) || f2.name.endsWith(".ts") || f2.name.endsWith(".tsx") : true
            );
            ...
            for (let i2 = 0; i2 < filesToShow.length; i2++) {
              const file = filesToShow[i2];
              const isLast = i2 === filesToShow.length - 1 && hiddenCount === 0;
              const connector = isLast ? "\u2514\u2500\u2500 " : "\u251C\u2500\u2500 ";
        ...
        lines.push(path3.basename(rootPath) + "/");
        scan(rootPath, "", 0);
        return lines.join("\n");
      }
      /**
       ...
       */
      async _scanExistingTypes(rootPath) {
        const types = [];
        const typeDirs = ["src/types", "src/lib", "types", "lib"];
        for (const dir of typeDirs) {
          const dirPath = path3.join(rootPath, dir);
          if (!fs4.existsSync(dirPath)) continue;
          try {
            const files = fs4.readdirSync(dirPath).filter((f2) => f2.endsWith(".ts") || f2.endsWith(".tsx"));
            for (const file of files.slice(0, 10)) {
              const filePath = path3.join(dirPath, file);
              try {
                const content = fs4.readFileSync(filePath, "utf-8");
                const relativePath = path3.join(dir, file);
                const interfaceRegex = /export\s+interface\s+(\w+)/g;
                let match;
                while ((match = interfaceRegex.exec(content)) !== null) {
        ...
        for (const t2 of types) {
          const arr = byFile.get(t2.file) || [];
          arr.push(t2);
          byFile.set(t2.file, arr);
        }
        for (const [file, fileTypes] of byFile) {
          lines.push(`  ${file}:`);
          for (const t2 of fileTypes.slice(0, 5)) {
            lines.push(`    - ${t2.kind} ${t2.name}`);
          }
       ...
       */
      async _loadAIMemory(rootPath) {
        const memoryPath = path3.join(rootPath, ".codebakers", "memory.json");
       ...
       */
      async _saveAIMemory(rootPath, memory) {
        const codebakersDir = path3.join(rootPath, ".codebakers");
        const memoryPath = path3.join(codebakersDir, "memory.json");
        if (!fs4.existsSync(codebakersDir)) {
          fs4.mkdirSync(codebakersDir, { recursive: true });
        }
        memory.lastUpdated = (/* @__PURE__ */ new Date()).toISOString();
        fs4.writeFileSync(memoryPath, JSON.stringify(memory, null, 2));
      }
      /**
       * Add a memory item to the AI memory
       */
      async addMemory(category, content, source = "inferred", confidence = 0.7) {
        const workspaceFolder = vscode5.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) return;
        const rootPath = workspaceFolder.uri.fsPath;
        ...
        const existing = memory[category].find(
          (m2) => this._similarContent(m2.content, content)
        );
        if (existing) {
        ...
        memory[category] = memory[category].sort((a2, b2) => b2.confidence - a2.confidence).slice(0, 20);
       ...
       */
      _similarContent(a2, b2) {
        const normalize = (s2) => s2.toLowerCase().replace(/[^a-z0-9]/g, "");
        const na = normalize(a2);
        const nb = normalize(b2);
        if (na.includes(nb) || nb.includes(na)) return true;
        const words1 = new Set(a2.toLowerCase().split(/\s+/));
        const words2 = new Set(b2.toLowerCase().split(/\s+/));
        const intersection = [...words1].filter((w2) => words2.has(w2));
      ...
      formatMemoryForPrompt(memory) {
        const sections = ["## \u{1F9E0} AI Project Memory (learned from previous sessions)"];
        const formatCategory = (items, title, emoji) => {
          const highConfidence = items.filter((m2) => m2.confidence >= 0.5);
...
### ${emoji} ${title}`);
        ...
        formatCategory(memory.architecture, "Architecture Decisions", "\u{1F3D7}\uFE0F");
        formatCategory(memory.preferences, "User Preferences", "\u2699\uFE0F");
        formatCategory(memory.keyFiles, "Key Files", "\u{1F4C1}");
        formatCategory(memory.patterns, "Common Patterns", "\u{1F504}");
       ...
       */
      async learnFromResponse(response, userMessage) {
        const memoryPatterns = [
          { pattern: /REMEMBER:\s*(.+?)(?:\n|$)/gi, category: "architecture" },
          { pattern: /USER PREFERS:\s*(.+?)(?:\n|$)/gi, category: "preferences" },
          { pattern: /KEY FILE:\s*(.+?)(?:\n|$)/gi, category: "keyFiles" },
          { pattern: /PATTERN:\s*(.+?)(?:\n|$)/gi, category: "patterns" },
          { pattern: /AVOID:\s*(.+?)(?:\n|$)/gi, category: "avoid" }
        ];
        ...
        if (userMessage.toLowerCase().includes("don't") || userMessage.toLowerCase().includes("never") || userMessage.toLowerCase().includes("stop")) {
          const avoidMatch = userMessage.match(/(?:don't|never|stop)\s+(.+?)(?:\.|$)/i);
          if (avoidMatch) {
            await this.addMemory("avoid", avoidMatch[1].trim(), "user", 0.9);
          }
        }
        if (userMessage.toLowerCase().includes("always") || userMessage.toLowerCase().includes("prefer")) {
      ...
      async clearMemory() {
        const workspaceFolder = vscode5.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) return;
        const memoryPath = path3.join(workspaceFolder.uri.fsPath, ".codebakers", "memory.json");
        if (fs4.existsSync(memoryPath)) {
          fs4.unlinkSync(memoryPath);
        }
        this._cache = null;
      }
    };
...
var ChatPanelProvider2;
var CodeBakersClient2;
var ProjectContext2;
var DiffContentProvider2;
var chatPanel;
var client;
var projectContext;
var statusBarItem;
...
async function loadModules() {
  try {
    ...
    const chatModule = await Promise.resolve().then(() => (init_ChatPanelProvider(), ChatPanelProvider_exports));
    ChatPanelProvider2 = chatModule.ChatPanelProvider;
    console.log("CodeBakers: ChatPanelProvider loaded");
    console.log("CodeBakers: Loading CodeBakersClient...");
    const clientModule = await Promise.resolve().then(() => (init_CodeBakersClient(), CodeBakersClient_exports));
    CodeBakersClient2 = clientModule.CodeBakersClient;
    console.log("CodeBakers: CodeBakersClient loaded");
    console.log("CodeBakers: Loading ProjectContext...");
    const contextModule = await Promise.resolve().then(() => (init_ProjectContext(), ProjectContext_exports));
    ProjectContext2 = contextModule.ProjectContext;
    console.log("CodeBakers: ProjectContext loaded");
    console.log("CodeBakers: Loading FileOperations...");
    const fileOpsModule = await Promise.resolve().then(() => (init_FileOperations(), FileOperations_exports));
    DiffContentProvider2 = fileOpsModule.DiffContentProvider;
...
async function handleSelectionCommand(action) {
  ...
  const selection = editor.selection;
  if (selection.isEmpty) {
    vscode6.window.showWarningMessage("No text selected");
    return;
  }
  const selectedText = editor.document.getText(selection);
  const fileName = editor.document.fileName;
  const relativePath = vscode6.workspace.asRelativePath(fileName);
...
\`\`\``;
  ...
  try {
    ...
    console.error("CodeBakers: Error handling selection command:", e2);
    vscode6.window.showErrorMessage(`CodeBakers error: ${e2}`);
  }
}
async function ensureInitialized() {
  ...
  if (!ChatPanelProvider2 || !CodeBakersClient2 || !ProjectContext2) {
    console.log("CodeBakers: Modules not loaded, loading now...");
    const loaded = await loadModules();
    if (!loaded) {
      vscode6.window.showErrorMessage(`CodeBakers: ${initializationError}`);
      return false;
    }
  }
  try {
...
function activate(context) {
  ...
  try {
    ...
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.openChat", async () => {
        console.log("CodeBakers: openChat command executed");
        const initialized = await ensureInitialized();
        if (!initialized) {
          vscode6.window.showErrorMessage("CodeBakers failed to initialize. Please try reloading VS Code.");
          return;
        }
        try {
          if (!client.hasSessionToken()) {
            vscode6.window.showWarningMessage(
              "\u{1F36A} Sign in to CodeBakers to start your free trial",
              "Sign In with GitHub"
            ).then((selection) => {
              if (selection === "Sign In with GitHub") {
                client.login();
          ...
          console.error("CodeBakers: Error in openChat:", e2);
          vscode6.window.showErrorMessage(`CodeBakers error: ${e2}`);
        }
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.login", async () => {
        ...
        try {
          ...
          console.error("CodeBakers: Login error:", e2);
          vscode6.window.showErrorMessage(`Login failed: ${e2}`);
        }
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.logout", async () => {
        ...
        try {
          ...
          console.error("CodeBakers: Logout error:", e2);
        }
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.showPatterns", async () => {
        const initialized = await ensureInitialized();
        if (!initialized) return;
        try {
          const patterns = await client.getAvailablePatterns();
          const quickPick = vscode6.window.createQuickPick();
          quickPick.items = patterns.map((p2) => ({ label: p2.name, description: p2.description }));
          quickPick.title = "Available CodeBakers Patterns";
          quickPick.show();
        } catch (e2) {
          console.error("CodeBakers: Error showing patterns:", e2);
        }
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.runAudit", async () => {
        const initialized = await ensureInitialized();
        if (!initialized) return;
        try {
          chatPanel.show();
          ...
          console.error("CodeBakers: Error running audit:", e2);
        }
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.askAboutSelection", async () => {
        console.log("CodeBakers: askAboutSelection command executed");
        await handleSelectionCommand("ask");
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.explainSelection", async () => {
        console.log("CodeBakers: explainSelection command executed");
        await handleSelectionCommand("explain");
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.refactorSelection", async () => {
        console.log("CodeBakers: refactorSelection command executed");
        await handleSelectionCommand("refactor");
      })
    );
    context.subscriptions.push(
      vscode6.commands.registerCommand("codebakers.addTestsForSelection", async () => {
    ...
    vscode6.window.showErrorMessage(`CodeBakers FATAL: Failed to register commands - ${error}`);
    return;
  }
  try {
    ...
    console.error("CodeBakers: Status bar failed:", e2);
  }
  try {
    if (DiffContentProvider2) {
      const diffProvider = new DiffContentProvider2();
      context.subscriptions.push(
        vscode6.workspace.registerTextDocumentContentProvider("codebakers-diff", diffProvider)
      );
      console.log("CodeBakers: Diff content provider registered");
    }
  } catch (e2) {
    console.error("CodeBakers: Diff provider registration failed:", e2);
  }
  try {
    context.subscriptions.push(
      vscode6.window.registerUriHandler({
        handleUri: async (uri) => {
          ...
          try {
            ...
            if (error) {
              vscode6.window.showErrorMessage(`Login failed: ${params.get("message") || error}`);
              return;
            }
            if (encodedToken) {
              ...
              vscode6.window.showErrorMessage("Login failed: No token received");
            }
          } catch (e2) {
            console.error("CodeBakers: URI handler error:", e2);
            vscode6.window.showErrorMessage(`OAuth error: ${e2}`);
          }
        }
      })
    );
    ...
    console.error("CodeBakers: URI handler registration failed:", e2);
  }
  setTimeout(async () => {
    try {
      const initialized = await ensureInitialized();
      if (initialized) {
        ...
        if (!client.hasSessionToken()) {
          vscode6.window.showInformationMessage(
            "\u{1F36A} CodeBakers installed! Click the CodeBakers button in the status bar to get started.",
            "Sign In Now"
          ).then((selection) => {
        ...
        try {
          ...
          console.warn("CodeBakers: File watcher failed:", e2);
        }
      }
    } catch (e2) {
      console.error("CodeBakers: Background init failed:", e2);
    }
  }, 500);
...
function updateStatusBar() {
  ...
  try {
    if (client?.hasSessionToken()) {
      ...
      statusBarItem.backgroundColor = new vscode6.ThemeColor("statusBarItem.warningBackground");
    }
  } catch (e2) {
    statusBarItem.text = "$(code) CodeBakers";
  }
